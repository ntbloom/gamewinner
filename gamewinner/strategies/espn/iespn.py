...
ESPN_FILE = Path(__file__).parent.joinpath("data").joinpath("ESPN.csv")


class ESPNProps(NamedTuple):
    rank: int
    obpr: float
    dbpr: float
    bpr: float
...
class ESPNStrategy(IStrategy):
    ...
    """
     ...
     espn_teams: dict[str, ESPNProps]

    def prepare(self, teams: dict[str, Team]) -> None:
        self.espn_teams = self.load_data(teams)

    @staticmethod
    def load_data(teams: dict[str, Team]) -> teams: dict[str, ESPNProps]:
        """
        Loads all the Evan Miya data and attaches it to the teams
        """
        assert ESPN_FILE.exists()
        espn_teams = {}
    ...
    def get_props(self, team: Team) -> dict[str, ESPNProps]:
        return {espn = self.espn_teams[team.name]}
