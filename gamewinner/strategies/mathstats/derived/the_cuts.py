...
class TheCuts23(IMathStatsStrategy):
    ...
    Additional weighting on Roster Rank and a little extra random bonus
    weighted on the inverse of Home Rank (i.e. you get a slight penalty
    for being significantly better at home than on the road). Note: this
    was initially on Resume Rank, but that got removed from the data in 2024.
    """
...
class TheCuts23Frozen(IMathStatsStrategy):
    ...
    def _team_metric(self, team: Team) -> float:
        ...
        overall_score = self._rank_to_percentile(props.rank_roster) * props.raw_overall
        #
        # # upset factor
        overall_score = overall_score + (
